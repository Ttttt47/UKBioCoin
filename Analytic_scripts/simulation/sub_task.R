# setwd("/data/jingcheng/project/UKBioCoin/simulation")
source("functions.R")
# ---- 4. Main ----
...
out_pref = paste0(trait, "_", sig_maf, "_", h2, "_", mcar)
...
for (b in B_start:B_end){
    ...
    ids <- data.table(FID = 1:nrow(sim$G),
                    IID = 1:nrow(sim$G))
    PHENO <- sim$Y
    if (trait == "binary") PHENO <- PHENO + 1   # 0/1  → 1/2
    phc <- cbind(ids, PHENO = PHENO, sim$covariates)
